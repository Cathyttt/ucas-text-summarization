...
def test():
    ...
    test_iter = DataLoader(dataset=test_dataset,
                           ...
                           shuffle=False)
    if use_gpu:
        checkpoint = torch.load(args.load_dir, map_location=lambda storage, loc: storage.cuda(0))
    else:
...
def predict(examples):
    ...
    pred_iter = DataLoader(dataset=pred_dataset,
                           batch_size=args.batch_size,
                           shuffle=False)
    args.load_dir = "/mnt/sdg/tyt/ucas-ts/SummaRuNNer/checkpoints/RNN_RNN_seed_1.pt"
