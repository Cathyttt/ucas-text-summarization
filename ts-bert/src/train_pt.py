...
def run(args, device_id, error_queue):
    ...
    try:
        ...
        train_pt_single(args, device_id)
...
def validate_pt(args, device_id):
    timestep = 0
    if (args.test_all):
...
def validate(args, device_id, pt, step):
    ...
    model = PTSummarizer(args, device, checkpoint)
    model.eval()
...
def test_pt(args, device_id, pt, step):
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
...
def train_pt_single(args, device_id):
    ...
    model = PTSummarizer(args, device, checkpoint, bert_from_extractive)
    if (args.sep_optim):
